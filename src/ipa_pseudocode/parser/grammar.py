...
class Parser:
    ...
    def _consume_terminator(self, keyword: str) -> None:
        """ブロック終端キーワード（endfor/endwhile/endif）を消費する"""
        if not self._at_end() and self._current_line().lower().strip() == keyword:
            self._advance_line()
    ...
    def _parse_block(
        ...
        ターミネータ（endif/endfor/endwhile）は消費しない。
        呼び出し元の _parse_if/_parse_for/_parse_while が消費する。
        """
        stmts: list[Any] = []
        while not self._at_end():
            line = self._current_line()
    ...
    def _parse_for(self) -> ForStatement | ForEachStatement:
        ...
        if m := _FOR_STANDARD_RE.match(line):
            ...
            self._consume_terminator("endfor")
            return ForStatement(
                var_name=m.group(1),
    ...
    def _parse_while(self) -> WhileStatement:
        ...
        condition = self._parse_condition(m.group(1))
        body = self._parse_block()
        self._consume_terminator("endwhile")
...
class ExpressionParser:
    ...
    _POSTFIX_PATTERNS = (
        re.compile(r"^の\d+乗$"),
        re.compile(r"^の正の平方根の整数部分$"),
        re.compile(r"^の正の平方根$"),
        re.compile(r"^の.+文字目の文字$"),
    )

    def _is_postfix_suffix(self, value: str) -> bool:
        """現在のトークンが後置パターンかどうかを判定する"""
        if value in self._PROPERTY_SUFFIXES:
            return True
        return any(p.match(value) for p in self._POSTFIX_PATTERNS)
    ...
    def parse(self, min_prec: int = 0) -> Expression:
        ...
        while not self._at_end():
            if self._current_precedence() > min_prec:
                ...
                and self._is_postfix_suffix(self._current().value)
            ):
                left = self._parse_infix(left)
            else:
                break
    ...
    def _parse_prefix(self) -> Expression:
        ...
        if tok.type == TokenType.IDENTIFIER:
            ...
            if m:
                return CharAt(
                    string=self._parser._parse_expression(m.group(1)),
                    index=self._parser._parse_expression(m.group(2)),
                )

            # 数学パターン: Xの正の平方根の整数部分 → int(X ** 0.5)
            m = re.match(r"^(.+?)の正の平方根の整数部分$", name)
            if m:
                return FunctionCall(
                    ...
                    arguments=[
                        BinaryOp(
                            ...
                            right=RealLiteral(value=0.5),
                        )
                    ],
                )

            # 数学パターン: Xの正の平方根 → X ** 0.5
            m = re.match(r"^(.+?)の正の平方根$", name)
            if m:
                return BinaryOp(
                    op="**",
                    left=self._parser._parse_expression(m.group(1)),
    ...
    def _parse_infix(self, left: Expression) -> Expression:
        ...
        if tok.type == TokenType.IDENTIFIER:
            ...
            # N乗: exprのN乗 → expr ** N
            m = re.match(r"^の(\d+)乗$", tok.value)
            if m:
                self._advance()
                return BinaryOp(
                    op="**",
                    left=left,
                    right=IntegerLiteral(value=int(m.group(1))),
                )

            # 正の平方根の整数部分: exprの正の平方根の整数部分 → int(expr ** 0.5)
            if tok.value == "の正の平方根の整数部分":
                self._advance()
                return FunctionCall(
                    function=Identifier(name="int"),
                    arguments=[
                        BinaryOp(op="**", left=left, right=RealLiteral(value=0.5))
                    ],
                )

            # 正の平方根: exprの正の平方根 → expr ** 0.5
            if tok.value == "の正の平方根":
                self._advance()
                return BinaryOp(op="**", left=left, right=RealLiteral(value=0.5))

            # CharAt: exprのY文字目の文字 → expr[Y - 1]
            m = re.match(r"^の(.+?)文字目の文字$", tok.value)
            if m:
                self._advance()
                return CharAt(
                    string=left,
                    index=self._parser._parse_expression(m.group(1)),
                )
