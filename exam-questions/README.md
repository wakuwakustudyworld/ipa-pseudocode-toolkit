...
- **令和6年春 午後**: 令和6年春 応用情報技術者試験 午後 問3
- **令和6年秋 午後**: 令和6年秋 応用情報技術者試験 午後 問3
- **令和7年春 午後**: 令和7年春 応用情報技術者試験 午後 問3
- **令和7年秋 午後**: 令和7年秋 応用情報技術者試験 午後 問3
...
AP問題はクラス・メソッド呼び出し・複合条件・0-based配列など、自動変換では対応困難な構文を多用するため、すべて手動でPythonに変換した。

| ファイル | 概要 | 手動変換の理由 |
|---------|------|---------------|
| `ap/2024r06h/q03.py` | ダイクストラ法 | 複合条件（`かつ`）、外部変数、二次元配列 |
| `ap/2024r06a/q03.py` | エラトステネスの篩 | 3関数の段階的改良、整数除算（`÷...の商`）のインデックス計算 |
| `ap/2025r07h/q03.py` | スライドパズル（BFS） | クラス（BoardState, Queue, List）、メソッド呼び出し、ディープコピー |
| `ap/2025r07a/q03.py` | 最長共通部分列（LCS） | 0-based配列、二次元配列の動的初期化 |
...
- AP問題の `.py` ファイルは標準Pythonのリストや独自クラスを使用しており、`Array` クラスは使用していない
